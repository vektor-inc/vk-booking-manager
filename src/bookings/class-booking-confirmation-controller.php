 ...
 */
    ...
    /**
     * Finalize a reservation draft into a booking post.
     *
    ...
    public function create_booking( WP_REST_Request $request ) {
        ...
        if ( '' === $token ) {
            return new WP_Error(
                'missing_token',
                __( 'Draft token not found.', 'vk-booking-manager' ),
                [ 'status' => 400 ]
            );
        }
        ...
        $slot     = isset( $draft['slot'] ) && is_array( $draft['slot'] ) ? $draft['slot'] : [];
		...
		if ( $menu_id <= 0 || '' === $start_at ) {
			return new WP_Error(
				'invalid_draft',
                __( 'Draft contents are incomplete.', 'vk-booking-manager' ),
                [ 'status' => 400 ]
			);
		}
		...
		$assignable_staff = $this->normalize_assignable_staff_ids( $available_slot['assignable_staff_ids'] ?? [] );
	...
	/**
	 * Revalidate draft slot availability before booking confirmation.
	 *
	 * 予約確定前にドラフトの枠がまだ空いているか再検証します。
	 *
	 * @param int                  $menu_id            Menu post ID.
	 * @param int                  $preferred_staff_id Preferred staff ID (0 for auto).
	 * @param array<string, mixed> $slot               Draft slot payload.
	...
	private function revalidate_draft_slot(
		...
		// Resolve timezone from draft or slot data. / ドラフトまたは枠情報からタイムゾーンを補正します。
		...
		$slots = isset( $availability['slots'] ) && is_array( $availability['slots'] ) ? $availability['slots'] : [];
	 ...
	 * @param string                           $slot_id Draft slot ID.
	 * @param string                           $start_at Draft start datetime.
	 * @param string                           $end_at Draft end datetime.
