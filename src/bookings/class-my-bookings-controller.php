...
class My_Bookings_Controller {
	...
	public function cancel_booking( WP_REST_Request $request ) {
		...
		if ( $booking_id <= 0 ) {
			return new WP_Error( 'invalid_booking', __( 'The reservation ID is invalid.', 'vk-booking-manager' ), [ 'status' => 400 ] );
		}

		$post = get_post( $booking_id );
		if ( ! $post instanceof WP_Post || Booking_Post_Type::POST_TYPE !== $post->post_type ) {
			return new WP_Error( 'booking_not_found', __( 'No reservations found.', 'vk-booking-manager' ), [ 'status' => 404 ] );
		}

		if ( (int) $post->post_author !== $user_id ) {
			return new WP_Error( 'forbidden', __( 'You do not have permission to cancel this reservation.', 'vk-booking-manager' ), [ 'status' => 403 ] );
		}

		$start_at = sanitize_text_field( (string) get_post_meta( $booking_id, self::META_SERVICE_START, true ) );
		$start_ts = $start_at ? strtotime( $start_at ) : false;
		if ( false === $start_ts ) {
			return new WP_Error( 'invalid_booking', __( 'The reservation date and time is invalid.', 'vk-booking-manager' ), [ 'status' => 400 ] );
		}
		...
		if ( ! $this->can_cancel_booking( (int) $start_ts, $now, $status, $cancel_mode, $cancel_hours ) ) {
			if ( 'none' === $cancel_mode ) {
				return new WP_Error( 'cancel_disabled', __( 'This reservation cannot be canceled online.', 'vk-booking-manager' ), [ 'status' => 403 ] );
			}
			return new WP_Error( 'cancel_deadline_passed', __( 'The cancellation deadline has passed.', 'vk-booking-manager' ), [ 'status' => 403 ] );
		}
