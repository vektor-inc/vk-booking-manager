...
class Settings_Sanitizer {
	private const HOLIDAY_FREQUENCIES = [
		'weekly',
		'nth-1',
		'nth-2',
		'nth-3',
	...
	private const BUSINESS_DAY_KEYS = [
		'mon',
		'tue',
		'wed',
		'thu',
	...
	public function sanitize( array $input, array $defaults ): array {
		...
		$data['resource_label_menu']     = sanitize_text_field( (string) ( $data['resource_label_menu'] ?? '' ) );
			...
			$data['provider_booking_status_mode'] = in_array( $booking_status_mode, [ 'confirmed', 'pending' ], true ) ? $booking_status_mode : 'confirmed';
		$cancel_mode = sanitize_key( (string) ( $input['provider_booking_cancel_mode'] ?? ( $data['provider_booking_cancel_mode'] ?? 'hours' ) ) );
		$data['provider_booking_cancel_mode'] = in_array( $cancel_mode, [ 'hours', 'none' ], true ) ? $cancel_mode : 'hours';
		...
		$data['provider_privacy_policy_mode'] = in_array( $privacy_mode, [ 'none', 'url', 'content' ], true ) ? $privacy_mode : 'none';
	...
	private function sanitize_business_hours_weekly(
		...
		foreach ( $sanitized as $day_key => $default_values ) {
			...
			$sanitized[ $day_key ] = [
				...
				'time_slots' => $should_use_custom ? $time_slots : [],
			];
		}
	...
	private function sanitize_time_components( array $input, string $prefix, string $context_label, array $field_path ): string {
		...
		if ( ! in_array( $minute_int, [ 0, 10, 20, 30, 40, 50 ], true ) ) {
			$this->add_error(
				sprintf(
					/* translators: %s: context label. */
	 ...
	 */
	private function add_error( string $message, array $field_path = [] ): void {
		$this->errors[] = $message;
