...
if ( edition !== 'free' && edition !== 'pro' ) {
	console.error(
		'Usage: node bin/switch-resource-config.js <free|pro> [distDirName]'
	);
	process.exit( 1 );
}
...
if ( edition === 'free' ) {
	...
	const deactivatorPath = path.join(
		distRoot,
		'class-free-version-deactivator.php'
	);
	if ( fs.existsSync( deactivatorPath ) ) {
		fs.unlinkSync( deactivatorPath );
		console.log(
			'Removed class-free-version-deactivator.php from free edition'
		);
	}
