...
const reservationComponentsDir = path.join(
	__dirname,
	'..',
	'src',
	'blocks',
...
if (
	fs.existsSync( bookingUiDir ) ||
	fs.existsSync( reservationComponentsDir )
) {
	// 自動的に booking-ui ディレクトリ内の全ての .js ファイルを検出（index.js は除外）
	const bookingUiFiles = fs.existsSync( bookingUiDir )
		? fs
				.readdirSync( bookingUiDir )
				.filter(
					( file ) => file.endsWith( '.js' ) && file !== 'index.js'
				)
				.map(
					( file ) => `src/blocks/reservation/booking-ui/${ file }`
				)
		: [];

	// 自動的に components ディレクトリ内の全ての .js ファイルを検出（index.js は除外）
	const reservationComponentFiles = fs.existsSync( reservationComponentsDir )
		? fs
				.readdirSync( reservationComponentsDir )
				.filter(
					( file ) => file.endsWith( '.js' ) && file !== 'index.js'
				)
				.map(
					( file ) => `src/blocks/reservation/components/${ file }`
				)
		: [];
	...
	const sourcesToMerge = [
		...bookingUiFiles,
		...reservationComponentFiles,
		...reservationBundledFiles,
	];
	...
	for ( const file of jsonFiles ) {
		...
		if (
			data.source &&
			data.source.includes( 'src/blocks/reservation/app.js' )
		) {
			appJsFile = { ...file, data };
			break;
		}
	...
	if ( appJsFile ) {
		...
		for ( const source of sourcesToMerge ) {
			...
			if ( bookingUiFile ) {
				const bookingUiData = JSON.parse(
					fs.readFileSync( bookingUiFile.path, 'utf8' )
				);
				const bookingUiMessages =
					bookingUiData.locale_data?.messages || {};
				...
				console.log(
					`  Merged translations from ${ path.basename( source ) }`
				);
			} else {
				console.log(
					`  Skipped ${ path.basename(
						source
					) } (no JSON generated; likely no translatable strings)`
				);
			}
		}
		...
		if ( mergedCount > 0 ) {
			...
			fs.writeFileSync(
				appJsFile.path,
				JSON.stringify( appJsFile.data, null, '\t' )
			);
			console.log(
				`Merged ${ mergedCount } translation entries into ${ appJsFile.filename }`
			);
			totalMergedCount += mergedCount;
		} else {
			console.log(
				'No reservation translations to merge (all translations may already be merged)'
			);
		}
	} else {
		console.log(
...
const menuLoopIndexJsFile = jsonFiles.find( ( file ) => {
	...
	const data = JSON.parse( content );
	return (
		data.source && data.source.includes( 'src/blocks/menu-loop/index.js' )
	);
} );
...
const menuLoopEditJsFile = jsonFiles.find( ( file ) => {
	...
	const data = JSON.parse( content );
	return (
		data.source && data.source.includes( 'src/blocks/menu-loop/edit.js' )
	);
} );
...
if ( menuLoopIndexJsFile && menuLoopEditJsFile ) {
	...
	const indexJsData = JSON.parse(
		fs.readFileSync( menuLoopIndexJsFile.path, 'utf8' )
	);
	const editJsData = JSON.parse(
		fs.readFileSync( menuLoopEditJsFile.path, 'utf8' )
	);
	...
	if ( mergedCount > 0 ) {
		...
		fs.writeFileSync(
			menuLoopIndexJsFile.path,
			JSON.stringify( indexJsData, null, '\t' )
		);
		console.log(
			`Merged ${ mergedCount } translation entries from edit.js into ${ menuLoopIndexJsFile.filename }`
		);
		totalMergedCount += mergedCount;
	} else {
		console.log(
			'No menu-loop translations to merge (all translations may already be merged)'
		);
	}
} else {
